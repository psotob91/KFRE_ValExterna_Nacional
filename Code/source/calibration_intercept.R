...
calibration_intercept <- function(data, horizon, primary_event, pred) {
  ...
  return(data.frame(estimate = summary(fit_cal_int)$mean$estimate, 
                    se = summary(fit_cal_int)$mean$san.se, 
                    var = summary(fit_cal_int)$mean$san.se ^ 2))
}