...
```{r}
glimpse(data_dial_sig)
```
```{r}
data_dial_sig |> count(`TIPO DE DOC`)
```
...
data_dial_sig2 <- data_dial_sig |> 
  ...
  mutate(auto_cor = str_trim(auto_cor), 
         ...
         tipo_doc = case_when(tipo_doc %in% c("L", "D") ~ "DNI/LE",
                              TRUE ~ "Otro")) |> 
  select(tipo_doc, dni, auto_cor, dial_date)
...
```{r}
glimpse(data_fallec_sinadef)
```

```{r}
data_fallec_sinadef  <- data_fallec_sinadef  |> 
...
data_pred_reba <- import(here("Data", "Raw", "bd_predictores_reba_lima.csv")) |> 
  clean_names() |> 
  mutate(cas_fuente = "JB")
```

```{r}
glimpse(data_pred_reba)
```
...
Notar que los datos del Rebagliati no contienen la vigilancia que hace el Hospital Kaelin, que pertenece a la Red Rebagliati, debido a que es una Asociacion Publico Privada con su propio manejo de Salud Renal.
...
##### Recuperar datos de DNI

```{r}
data_fallec_dni <- data_fallec |> 
  mutate(dni = case_when(dni %in% c("", "----") ~ as.character(NA), 
                         TRUE ~ dni)) |> 
  select(-death_date, -fuente) |> 
  arrange(dni)
```
...
```{r}
data_dial2_dni <- data_dial2 |> 
  mutate(dni = case_when(dni %in% c("", "----") ~ as.character(NA), 
                         TRUE ~ dni)) |> 
  select(-dial_date, -diag_cie) |> 
  arrange(dni)
...
data_pred_reba3 <- data_pred_reba2 |> 
  left_join(data_fallec_dni, 
            by = "auto_cor") |>   
  left_join(data_dial2_dni |> select(-dial), 
            by = "auto_cor")   

glimpse(data_pred_reba3)
```

```{r}
data_pred_reba3 <- data_pred_reba3 |> 
  mutate(dif_dni = case_when(dni.y == dni.x ~ "iguales", 
                             dni.y != dni.x ~ "diferentes"))

data_pred_reba3 |> 
  count(dif_dni)
```

En el caso de los DNI diferentes, vemos que podemos recsatar los valores

```{r}
data_pred_reba3 |> 
  filter(dif_dni == "diferentes") 
```

```{r}
data_pred_reba3 |> 
  filter(dif_dni == "iguales") 
```

En este caso, una de las dos filas tiene dni o ninguna ,asi que es posible recuperar los datos de dni para reutilizarlos luego:


```{r}
data_pred_reba3 |> 
  filter(is.na(dif_dni)) |> 
  arrange(dni.y) 
```

```{r}
data_pred_reba3 <- data_pred_reba3 |> 
  mutate(dni = case_when(dif_dni == "diferentes" & dni.x == "----" ~ dni.y, 
                         dif_dni == "diferentes" & dni.x != "----" ~ dni.x, 
                         dif_dni == "iguales" ~ dni.x, 
                         is.na(dif_dni) & !is.na(dni.x) ~ dni.x, 
                         is.na(dif_dni) & !is.na(dni.y) ~ dni.y, 
                         TRUE ~ dni.x))
```
...
##### Fusionar primero muerte


```{r}
data_pred_reba3 <- data_pred_reba3 |> 
  left_join(data_fallec |> select(-dni), 
            by = "auto_cor") |>   
  left_join(data_fallec |> select(-auto_cor), 
            by = "dni") |>   
  left_join(data_fallec_sinadef, 
            by = c("dni" = "documento"))
...
```{r}
data_pred_reba3b <- data_pred_reba3 |> 
  mutate(fech_fallecimiento = as.Date(fech_fallecimiento), 
         date_death_igual1 = (death_date.x == death_date.y ), 
         date_death_igual2 = (fech_fallecimiento == death_date.y))

 data_pred_reba3b |> 
   select(auto_cor, sex, age, death_date.x, death_date.y, death_date.y, 
          fech_fallecimiento, date_death_igual1, date_death_igual2) |> 
   arrange(date_death_igual1) |> 
   filter(date_death_igual1 == TRUE)
```
```{r}
 data_pred_reba3b |> 
   select(auto_cor, sex, age, death_date.x, death_date.y, death_date.y, 
          fech_fallecimiento, date_death_igual1, date_death_igual2) |> 
   arrange(date_death_igual1) |> 
   filter(date_death_igual1 == FALSE)
```
...
```{r}
 data_pred_reba3b |> 
   select(auto_cor, sex, age, death_date.x, death_date.y, death_date.y, 
          fech_fallecimiento, date_death_igual1, date_death_igual2) |> 
   arrange(date_death_igual1) |> 
   filter(is.na(date_death_igual1))
```
Respecto a death_date.x y death_date.y, ambos dan lo mismo, nos quedaremos con death_date.x

```{r}
 data_pred_reba3b |> 
   select(auto_cor, sex, age, death_date.x, death_date.y, death_date.y, 
          fech_fallecimiento, date_death_igual1, date_death_igual2) |> 
   arrange(date_death_igual2) |> 
   filter(date_death_igual2 == TRUE)
```
...
```{r}
 data_pred_reba3b |> 
   select(auto_cor, sex, age, death_date.x, death_date.y, death_date.y, 
          fech_fallecimiento, date_death_igual1, date_death_igual2) |> 
   arrange(date_death_igual2) |> 
   filter(date_death_igual2 == FALSE)
```
...
```{r}
 data_pred_reba3b |> 
   select(auto_cor, sex, age, death_date.x, death_date.y, death_date.y, 
          fech_fallecimiento, date_death_igual1, date_death_igual2) |> 
   arrange(date_death_igual2) |> 
   filter(is.na(date_death_igual2)) |> 
  arrange(fech_fallecimiento)
```

Por tanto vamos a editar la variable death_date. nos quedamos con death_date.x porque es igual que death_date.y. Luego, cuando sean diferentes, nos quedamos con fech_fallecimiento y si hay algun missing en uno, pero no en el otro, los intercambiamos.


```{r}
data_pred_reba3 <- data_pred_reba3b  |> 
  mutate(death_date = case_when(date_death_igual2 == FALSE ~ fech_fallecimiento, 
                                is.na(date_death_igual2) & 
                                  !is.na(death_date.x) ~ death_date.x, 
                                is.na(date_death_igual2) & 
                                  !is.na(fech_fallecimiento) ~ fech_fallecimiento, 
                                TRUE ~ as.Date(NA))
         ) |> 
  select(-death_date.x, -death_date.y, -date_death_igual1, 
         -date_death_igual2, -fech_fallecimiento)
```

Ahora que se cuenta con los datos corregidos, verificamos:

```{r}
data_pred_reba3 |> 
  count(death_date, .drop = FALSE) |> 
  arrange(death_date)
```
...
```{r}
data_pred_kaelin3b <- data_pred_kaelin2 |> 
  left_join(data_fallec, by = "dni")
```
...
```{r}
data_pred_kaelin3b |> 
  mutate(fallecido = if_else(!is.na(death_date), "si", "no")) |> 
  count(fallecido)
```
...
Se aprecia que no hay duplicados reconocidos de DNI, solo se identifican duplicados de datos perdidso de DNI>
