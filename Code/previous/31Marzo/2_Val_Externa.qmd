...
data <- complete(data_impA, action = "long", include = TRUE) |> 
  filter(.imp == 1) |> 
  mutate(id = .id)
rm(data_impA)
...
vdata <- dataA %>% 
  select(id, risk2y, risk5y, time5y, eventd5y, eventd5ylab, time, eventd) %>%  
  drop_na() %>%  
  mutate(status = eventd5ylab)
...
data %>%  
  filter(ckd_stage == group) |> 
  mutate(time0 = 0) -> data_filt
...
vdata.w <- crprep(
  ...
  trans = c(1, 2),
  id = "id",
  keep = c("age", "male", "eGFR_ckdepi", "acr", "risk2y", "risk5y"),
  data = data_filt
)
